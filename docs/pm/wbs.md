...
- [x] 1.1.1.1 Create project charter
- [x] 1.1.1.2 Develop WBS
- [x] 1.1.1.3 Create project schedule with milestones
- [x] 1.1.1.4 Create risk register

#### 1.1.2 Project Monitoring and Control
- [x] 1.1.2.1 Weekly self-review against schedule and risk register
- [x] 1.1.2.2 Update risk register as new risks emerge
- [x] 1.1.2.3 Track milestone progress; adjust schedule if needed
...
- [x] 1.2.1.1 Create Firebase project (dev + prod environments)
...
- [x] 1.2.2.1 Create AWS account and configure IAM roles and policies
- [x] 1.2.2.2 Create Lambda execution role with least-privilege permissions
- [x] 1.2.2.3 Configure Lambda layers (Java runtime dependencies, shared libraries)
- [x] 1.2.2.4 Set up environment variables and AWS Secrets Manager for API keys
- [x] 1.2.2.5 Configure CloudWatch alarms for Lambda errors and timeout events
- [x] 1.2.2.6 Set up EventBridge scheduled rule for weekly Lambda job trigger

#### 1.2.3 Google Places API
- [x] 1.2.3.1 Provision Google Cloud project and enable Places API
...
- [x] 1.2.4.1 Initialize monorepo structure (backend + iOS in single repository)
- [x] 1.2.4.2 Configure .gitignore, secrets handling (no secrets in source control)
- [x] 1.2.4.3 Set up backend project (Java, Maven or Gradle, Lambda handler scaffolding)
- [x] 1.2.4.4 Set up iOS project (Xcode project, dependency manager - Swift Package Manager)
- [x] 1.2.4.5 Configure CI pipeline (GitHub Actions or equivalent) for backend build and test
- [x] 1.2.4.6 Document local development setup (README)
...
- [x] 1.3.1.1 Implement Firebase ID token verification in Lambda (Java Firebase Admin SDK)
- [x] 1.3.1.2 Implement authentication middleware/filter applied to all protected endpoints
- [x] 1.3.1.3 Write unit tests for token verification (valid token, expired token, malformed token)

#### 1.3.2 User API
- [x] 1.3.2.1 Implement GET /me - return current user profile from Firestore
- [x] 1.3.2.2 Implement PATCH /me - update user profile fields (display name, city preference)
- [x] 1.3.2.3 Implement auto-provisioning of user document on first authenticated request
- [x] 1.3.2.4 Write unit tests for user read and update logic
- [x] 1.3.2.5 Write integration tests for GET /me and PATCH /me endpoints

#### 1.3.3 Feed API
- [x] 1.3.3.1 Implement GET /me/feed - return current weekly spot and 4-week history
- [x] 1.3.3.2 Include RSVP counts (yes, no, interested) per spot in feed response
- [x] 1.3.3.3 Include caller's own RSVP status per spot if they have responded
- [x] 1.3.3.4 Write unit tests for feed assembly logic
- [x] 1.3.3.5 Write integration tests for GET /me/feed

#### 1.3.4 RSVP API
- [x] 1.3.4.1 Implement POST /invites/{id}/rsvp - record RSVP response (yes/no/interested)
- [x] 1.3.4.2 Validate invite belongs to requesting user; reject if not
- [x] 1.3.4.3 Handle idempotent RSVP updates (user changes response)
- [x] 1.3.4.4 Write unit tests for RSVP write and validation logic
- [x] 1.3.4.5 Write integration tests for POST /invites/{id}/rsvp

#### 1.3.5 Push Token Registration API
- [x] 1.3.5.1 Implement POST /push-tokens - register or update FCM device token for authenticated user
- [x] 1.3.5.2 Handle token refresh (overwrite existing token for device)
...
- [x] 1.3.7.3 Manual end-to-end API test using real Firebase dev project and Postman/curl
- [x] 1.3.7.4 Review Firestore security rules - ensure users can only read their own data

#### 1.3.8 Backend Deployment
- [x] 1.3.8.1 Build deployment package (JAR or ZIP) and deploy Lambda functions to AWS
- [x] 1.3.8.2 Configure API Gateway (or Lambda Function URLs) for HTTP endpoint exposure
- [x] 1.3.8.3 Validate all endpoints respond correctly in deployed environment
- [x] 1.3.8.4 Configure EventBridge rule and verify weekly job triggers on schedule
- [x] 1.3.8.5 Confirm CloudWatch alarms are active and alerting correctly
...
- [x] 1.4.1.1 Build sign-in screen with Apple Sign-In button and Google Sign-In button
